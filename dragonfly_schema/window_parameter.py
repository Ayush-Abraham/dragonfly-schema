...
class SingleWindow(_WindowParameterBase):
    ...
    type: Annotated[str, StringConstraints(pattern="^SingleWindow$")] = "SingleWindow"
    ...
    sill_height: float = Field(
        1.0, gt=0, description="A number for the window sill height."
    )
...
class SimpleWindowArea(_WindowParameterBase):
    ...
    type: Annotated[str, StringConstraints(pattern="^SimpleWindowArea$")] = (
        "SimpleWindowArea"
    )
...
class SimpleWindowRatio(_WindowParameterBase):
    ...
    type: Annotated[str, StringConstraints(pattern="^SimpleWindowRatio$")] = (
        "SimpleWindowRatio"
    )
...
class RepeatingWindowRatio(_WindowParameterBase):
    ...
    type: Annotated[str, StringConstraints(pattern="^RepeatingWindowRatio$")] = (
...
class RepeatingWindowWidthHeight(_WindowParameterBase):
    ...
    type: Annotated[str, StringConstraints(pattern="^RepeatingWindowWidthHeight$")] = (
...
class RectangularWindows(_WindowParameterBase):
    ...
    type: Annotated[str, StringConstraints(pattern="^RectangularWindows$")] = (
        "RectangularWindows"
    )
    ...
    are_doors: List[bool] | None = Field(
        default=None,
    ...
    def check_aligned_lists(self):
        ...
        origins = self.origins  # values.get("origins")
        widths = self.widths  # values.get("widths")
        heights = self.heights  # values.get("heights")

        assert len(origins) == len(widths) == len(heights), (
            "Length of "
...
class DetailedWindows(_WindowParameterBase):
    ...
    type: Annotated[str, StringConstraints(pattern="^DetailedWindows$")] = (
        "DetailedWindows"
    )
