...
from honeybee_schema.boundarycondition import (
    Ground,
    Outdoors,
    Surface,
    Adiabatic,
...
from .shading_parameter import (
    ExtrudedBorder,
    Overhang,
    LouversByDistance,
    LouversByCount,
)
from .skylight_parameter import (
    GriddedSkylightArea,
...
class Room2DPropertiesAbridged(BaseModel):
    type: Annotated[str, StringConstraints(pattern="^Room2DPropertiesAbridged$")] = (
        "Room2DPropertiesAbridged"
    )

    energy: Room2DEnergyPropertiesAbridged | None = Field(default=None)

    radiance: Room2DRadiancePropertiesAbridged | None = Field(default=None)

    comparison: Room2DComparisonProperties | None = Field(default=None)


class Room2D(IDdBaseModel):
    type: Annotated[str, StringConstraints(pattern="^Room2D$")] = "Room2D"

    floor_boundary: List[Annotated[List[float], Field(min_length=2, max_length=2)]] = (
        Field(
            ...,
            min_length=3,
    ...
    boundary_conditions: (
        List[Union[Ground, Outdoors, Surface, Adiabatic, OtherSideTemperature]] | None
    ) = Field(
        default=None,
    ...
    shading_parameters: (
        List[Union[None, ExtrudedBorder, Overhang, LouversByDistance, LouversByCount]]
        | None
    ) = Field(
        default=None,
    ...
    air_boundaries: List[bool] | None = Field(
        default=None,
    ...
    def check_segment_count(self):
        ...
        floor_bound = self.floor_boundary  # values.get("floor_boundary")
        floor_holes = self.floor_holes  # values.get("floor_holes")
        bcs = self.boundary_conditions  # values.get("boundary_conditions")
        win_par = self.window_parameters  # values.get("window_parameters")
        shd_par = self.shading_parameters  # values.get("shading_parameters")
        air_bnd = self.air_boundaries  # values.get("air_boundaries")

        seg_count = (
            len(floor_bound)
            if floor_holes is None
            else len(floor_bound) + sum(len(hole) for hole in floor_holes)
        )

        if bcs is not None:
            assert len(bcs) == seg_count, (
                "Length of Room2D boundary_conditions "
                "must match number of floor segments. {} != {}".format(
                    len(bcs), seg_count
                )
            )
        if win_par is not None:
            assert len(win_par) == seg_count, (
                "Length of Room2D window_parameters "
                "must match number of floor segments. {} != {}".format(
                    len(win_par), seg_count
                )
            )
        if shd_par is not None:
            assert len(shd_par) == seg_count, (
                "Length of Room2D shading_parameters "
                "must match number of floor segments. {} != {}".format(
                    len(shd_par), seg_count
                )
            )
        if air_bnd is not None:
            assert len(air_bnd) == seg_count, (
                "Length of Room2D air_boundaries "
                "must match number of floor segments. {} != {}".format(
...
class StoryPropertiesAbridged(BaseModel):
    type: Annotated[str, StringConstraints(pattern="^StoryPropertiesAbridged$")] = (
        "StoryPropertiesAbridged"
    )

    energy: StoryEnergyPropertiesAbridged | None = Field(default=None)

    radiance: StoryRadiancePropertiesAbridged | None = Field(default=None)


class Story(IDdBaseModel):
    type: Annotated[str, StringConstraints(pattern="^Story$")] = "Story"
    ...
    roof: RoofSpecification | None = Field(
        default=None,
...
class BuildingPropertiesAbridged(BaseModel):
    type: Annotated[str, StringConstraints(pattern="^BuildingPropertiesAbridged$")] = (
        "BuildingPropertiesAbridged"
    )

    energy: BuildingEnergyPropertiesAbridged | None = Field(default=None)

    radiance: BuildingRadiancePropertiesAbridged | None = Field(default=None)


class Building(IDdBaseModel):
    type: Annotated[str, StringConstraints(pattern="^Building$")] = "Building"

    unique_stories: List[Story] | None = Field(
        default=None,
...
class ContextShadePropertiesAbridged(BaseModel):
    type: Annotated[
        str, StringConstraints(pattern="^ContextShadePropertiesAbridged$")
    ] = "ContextShadePropertiesAbridged"

    energy: ContextShadeEnergyPropertiesAbridged | None = Field(default=None)

    radiance: ContextShadeRadiancePropertiesAbridged | None = Field(default=None)


class ContextShade(IDdBaseModel):
    type: Annotated[str, StringConstraints(pattern="^ContextShade$")] = "ContextShade"
...
class ModelProperties(BaseModel):
    ...
    comparison: ModelComparisonProperties | None = Field(default=None)
...
class Model(IDdBaseModel):
    ...
    buildings: List[Building] | None = Field(
        None, description="A list of Buildings in the model."
    )

    context_shades: List[ContextShade] | None = Field(
        None, description="A list of ContextShades in the model."
    )
